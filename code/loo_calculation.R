...
# read the data
webs <- readr::read_csv("data/network_data.dat")

pred <- filter(webs, predation > 0)


## read models

betabin <- rstan::stan_model(file = "stan/betabin_connectance.stan", save_dso = TRUE)
conscon <- rstan::stan_model(file = "stan/constant_connect.stan", save_dso = TRUE)
lsslmod <- rstan::stan_model(file = "stan/lssl.stan", save_dso = TRUE)
powrlaw <- rstan::stan_model(file = "stan/powerlaw_connectance.stan", save_dso = TRUE)

# create a list of arguments

predsmall = filter(pred, links < 4e4)
...
betabin_samples <- rstan::sampling(betabin, data = d, iter = 2000, chains = 4, cores = 4)
...
betabin_r_eff <- relative_eff(exp(betabin_log_lik), cores = 4)

# preferably use more than 2 cores (as many cores as possible)
# will use value of 'mc.cores' option if cores is not specified
betabin_loo <- loo(betabin_log_lik, r_eff = betabin_r_eff, cores = 4)
print(betabin_loo)
